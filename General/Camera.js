var Camera = module.exports = function(ctx, dimensions, padding, controller){
  ...
  this.min = {x:dimensions.cur.x*dimensions.min, y:dimensions.cur.y*dimensions.min};
  this.max = {x:dimensions.cur.x*dimensions.max, y:dimensions.cur.y*dimensions.max};
  this.padding = {x:dimensions.cur.x*padding, y:dimensions.cur.y*padding};
...
Camera.prototype.step = function(timeStep){
  ...
  this.targets.forEach(function(target){
    ...
    if(target.desiredPosition){
      target.currentPosition.x += net * (target.desiredPosition.x - target.currentPosition.x);
      target.currentPosition.y += net * (target.desiredPosition.y - target.currentPosition.y);
...
//  console.log('box: ',dims.min, dims.max);
...
Camera.prototype.follow = function(body){
  if(this.following.indexOf(body) !== -1) throw new Error('already following this body');
